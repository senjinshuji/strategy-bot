...
import { EnhancedScrapedData, enhancedPlaywrightScraper } from './enhanced-playwright-scraper';
import { MarketingAnalysis, Product, Market, N1, Competitor, Concept } from '../types';
...
export class AutoAnalyzer {
  private provider = aiProviders.mock; // Default to mock

  setProvider(providerName: 'mock' | 'ollama' | 'claude' | 'chatgpt') {
    this.provider = aiProviders[providerName];
  }

  async analyzeFromUrl(url: string): Promise<AutoAnalysisResult> {
    try {
      // 1. High-quality scraping with Playwright
      console.log('🎯 高精度スクレイピング開始...');
      const scrapedData = await enhancedPlaywrightScraper.scrapeLP(url);
  ...
  private async convertToMarketingAnalysis(data: EnhancedScrapedData): Promise<MarketingAnalysis> {
  ...
  private convertToN1(profile: EnhancedScrapedData['n1Profile']): N1 {
    return {
      profile,
      journey: {
  ...
  private async analyzeProductOld(data: EnhancedScrapedData): Promise<Product> {
    const prompt = `
...
`;
  ...
  private async analyzeMarketOld(data: EnhancedScrapedData): Promise<Market> {
    const prompt = `
...
`;
  ...
  private async analyzeN1Old(data: EnhancedScrapedData): Promise<N1[]> {
    const prompt = `
...
`;
  ...
  private async analyzeCompetitorsOld(data: EnhancedScrapedData): Promise<Competitor> {
    const prompt = `
以下の製品の競合分析を行ってください：
...
`;
  ...
  private async analyzeConceptOld(data: EnhancedScrapedData): Promise<Concept> {
    const prompt = `
...
`;
  ...
  private extractProductCategory(analysis: string, data: EnhancedScrapedData): string {
    if (analysis.includes('SaaS') || analysis.includes('ソフトウェア')) return 'SaaS';
    if (analysis.includes('EC') || analysis.includes('通販')) return 'EC';
    if (analysis.includes('サービス') || analysis.includes('コンサル')) return 'サービス';
    return this.inferCategory(data);
  }

  private extractProductFeatures(analysis: string, data: EnhancedScrapedData): string[] {
    // AI analysis + scraped features
    return data.productInfo.functions.slice(0, 5);
  }

  private extractProductPricing(analysis: string, data: EnhancedScrapedData): string {
  ...
  private extractProductValue(analysis: string, data: EnhancedScrapedData): string[] {
    const values = [];
    if (data.description.includes('効率')) values.push('業務効率化');
    if (data.description.includes('コスト')) values.push('コスト削減');
    if (data.description.includes('品質')) values.push('品質向上');
    return values.length > 0 ? values : ['価値向上', '課題解決', '満足度向上'];
  }

  private extractMarketTarget(analysis: string, data: EnhancedScrapedData): string {
  ...
  private extractMarketSize(analysis: string, data: EnhancedScrapedData): string {
    return '中規模市場（推定）';
  }

  private extractMarketGrowth(analysis: string, data: EnhancedScrapedData): string {
    return '成長市場';
  }

  private extractMarketTrends(analysis: string, data: EnhancedScrapedData): string[] {
    return ['デジタル化推進', 'リモートワーク対応', '効率化ニーズ'];
  }

  private extractN1Persona(analysis: string, type: string, data: EnhancedScrapedData): N1 {
    return {
      profile: {
  ...
  private extractDirectCompetitors(analysis: string): string[] {
    return ['競合A', '競合B', '競合C'];
  }

  private extractIndirectCompetitors(analysis: string): string[] {
    return ['代替サービスA', '代替サービスB'];
  }

  private extractAlternatives(analysis: string): string[] {
    return ['手作業での対応', '他ツールの組み合わせ'];
  }

  private extractCompetitiveAdvantages(analysis: string, data: EnhancedScrapedData): string[] {
    const advantages = [];
  ...
  private extractBrandConcept(analysis: string, data: EnhancedScrapedData): string {
    return `${data.title}の価値を最大化する`;
  }

  private extractRTB(analysis: string, data: EnhancedScrapedData): string[] {
    const rtb = [];
    if (data.marketTarget.purchasers && data.marketTarget.purchasers !== '未確認') {
  ...
  private extractPositioning(analysis: string, data: EnhancedScrapedData): string {
    return `${this.inferCategory(data)}分野でのリーディングソリューション`;
  }

  private extractKeyMessage(analysis: string, data: EnhancedScrapedData): string {
    return data.description || `${data.title}で課題解決`;
  }

  // Utility methods
  private inferCategory(data: EnhancedScrapedData): string {
  ...
  private inferIndustry(data: EnhancedScrapedData): string {
    const content = (data.title + ' ' + data.description).toLowerCase();
    
    if (content.includes('医療') || content.includes('病院')) return '医療';
    if (content.includes('教育') || content.includes('学習')) return '教育';
    if (content.includes('不動産') || content.includes('物件')) return '不動産';
    if (content.includes('金融') || content.includes('投資')) return '金融';
    
    return 'IT・サービス';
  }

  private calculateConfidence(data: EnhancedScrapedData): number {
    // Use the confidence from enhanced scraper
    return data.metadata.confidence;
  }

  private generateSuggestions(data: EnhancedScrapedData, analysis: MarketingAnalysis): string[] {
    const suggestions = [];
  ...
  private getExtractionSources(data: EnhancedScrapedData): string[] {
    const sources = [];
    
    if (data.title) sources.push('ページタイトル');
    if (data.description) sources.push('メタ説明');
    if (data.productInfo.functions.length > 0) sources.push('機能一覧');
    if (data.productInfo.regularPrice) sources.push('価格情報');
    if (data.valueProposition.functionalValue.length > 0) sources.push('機能価値');
    if (data.valueProposition.emotionalValue.length > 0) sources.push('情緒価値');
    if (data.n1Profile.familyStructure) sources.push('N1プロフィール');
    if (data.competitors.categoryCompetitors.length > 0) sources.push('競合分析');
