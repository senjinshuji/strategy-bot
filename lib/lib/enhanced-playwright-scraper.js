...
exports.enhancedPlaywrightScraper = exports.EnhancedPlaywrightScraper = void 0;
...
class EnhancedPlaywrightScraper {
    ...
    async extractStructuredData(page, url, loadTime) {
    ...
    async extractProductInfo(page, fullText) {
        return await page.evaluate((fullText) => {
            ...
            // 共働き、忙しい、などの情報
            const otherInfo = [];
            if (fullText.includes('共働き'))
                otherInfo.push('共働き');
            if (fullText.includes('忙しい'))
                otherInfo.push('忙しいライフスタイル');
            if (fullText.includes('子育て'))
                otherInfo.push('子育て世代');
            if (fullText.includes('家族'))
                otherInfo.push('ファミリー');
            return {
                productName,
                category,
                sizeCapacity,
    ...
    async extractValueProposition(page, fullText) {
        return await page.evaluate((fullText) => {
    ...
    async extractMarketTarget(page, fullText) {
        return await page.evaluate((fullText) => {
            return {
    ...
    async extractN1Profile(page, fullText) {
        return await page.evaluate((fullText) => {
            // N1ペルソナの詳細分析
    ...
    async extractCompetitors(page, fullText) {
        return await page.evaluate((fullText) => {
            return {
    ...
    async extractConcept(page, fullText) {
        return await page.evaluate((fullText) => {
            return {
...
exports.EnhancedPlaywrightScraper = EnhancedPlaywrightScraper;
exports.enhancedPlaywrightScraper = new EnhancedPlaywrightScraper();
